...
```{r}
tb = df %>% 
  filter((Replicate %in% c(2,3,4,5)) & 
        (Treatment == 'Fungal') & (Mating_status == 'Cohabit') & (Sex=='M'))
sum(tb$Death)

# fit = survfit(Surv(Day, Death) ~ Cage_Number, data = tb)
# ggsurvplot(fit, surv.median.line = "hv", pval = TRUE)
# pairwise_survdiff(Surv(Day, Death == 1) ~ Cage_Number,
#                   p.adjust.method = "holm", data = tb)

survdiff(Surv(Day, Death) ~ Replicate , data = tb)
```
