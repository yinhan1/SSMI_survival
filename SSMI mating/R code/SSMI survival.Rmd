...
tb = df %>% mutate(Mating_status = factor(Mating_status, levels = level_mat), Rep = Replicate)
fit = survfit(Surv(Day, Death) ~ Rep, data = tb)
ggsurvplot(fit, surv.median.line = "hv", pval = TRUE)
pairwise_survdiff(Surv(Day, Death == 1) ~ Rep, p.adjust.method = "holm", data = tb)
...
```{r}
set.seed(57)
tb = df %>% filter((Replicate %in% c(2,3,4,5)) & (Mating_status != 'Virgin CO2')) %>%
  group_by(Replicate,Treatment,Mating_status,Sex) %>%
  slice_sample(n = 50, replace = TRUE)
t = survdiff(Surv(Day, Death) ~ Replicate + strata(Treatment) + strata(Mating_status) + strata(Sex), data = tb)
t
t$pvalue
```
...
```{r}
tb2 = df %>% filter(Replicate == 2, Mating_status != 'Virgin CO2')
tb3 = df %>% filter(Replicate == 3, Mating_status != 'Virgin CO2')
tb4 = df %>% filter(Replicate == 4, Mating_status != 'Virgin CO2')
tb5 = df %>% filter(Replicate == 5, Mating_status != 'Virgin CO2')
```
